...
static void hero_watcher_render(void *data, gs_effect_t *effect)
{
	...
	if (obs_source_process_filter_begin_with_color_space(filter->context, filter->color_format, filter->source_space,
...
static void hero_watcher_add(void *data, obs_source_t *source)
{
	...
	obs_enter_graphics();
		obs_source_t *target = obs_filter_get_target(filter->context);
		if (target) {
			filter->source_space = obs_source_get_color_space(
				target,
				OBS_COUNTOF(preferred_spaces),
				preferred_spaces
			);
			filter->color_format = gs_get_format_from_space(filter->source_space);
			float multi;
			filter->technique = get_tech_name_and_multiplier(
				gs_get_color_space(),
				filter->source_space,
				&multi
			);
			gs_effect_set_float(filter->param_multiplier, multi);
			
		} else {
			blog(LOG_ERROR, "[%s] No target found in filter_add", __func__);
		}
	obs_leave_graphics();
}
